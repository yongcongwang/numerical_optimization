...
namespace {
double cost(void *instance, const Eigen::VectorXd &x, Eigen::VectorXd &g) {
    const int n = x.size();
    double fx = 0.0;
    for (int i = 0; i < n; i += 2) {
...
void monitorProgress(void *instance, const Eigen::VectorXd &x,
...
bool Minimization::Run(const int n) {
  double finalCost;
  Eigen::VectorXd x(N);
